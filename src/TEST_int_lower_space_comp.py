...
V_H_sk = skfem.Basis(mesh_sk_H, skfem.ElementTriP2())
...
u_sk = skfem.solve(*skfem.condense(A_sk, b_sk, I = V_H_sk.get_dofs()))

mesh_sk_h = mesh_sk_H.refined(1)

V_h_sk = skfem.Basis(mesh_sk_h, skfem.ElementTriP1())
                 
I_H_u_sk = torch.tensor(V_h_sk.interpolate(u_sk))
I_H_u_grad_sk = torch.tensor(grad(V_h_sk.interpolate(u_sk)))

mesh_H = Mesh(torch.tensor(mesh_sk_H.p).T,  torch.tensor(mesh_sk_H.t).T)

V_H = Basis(mesh_H, Elements(P_order = 2, int_order = 2))
...
A = V_H.integrate_bilineal_form(a)[V_H.inner_dofs, :][:, V_H.inner_dofs]
b = V_H.integrate_lineal_form(l)[V_H.inner_dofs]

u = torch.zeros(V_H.nb_global_dofs, 1)
u[V_H.inner_dofs] = torch.linalg.solve(A,b)

mesh_h = Mesh(torch.tensor(mesh_sk_h.p).T, torch.tensor(mesh_sk_h.t).T)

V_h = Basis(mesh_h, Elements(P_order = 1, int_order = 2))
