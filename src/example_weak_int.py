...
V_H = Basis(
    Mesh_Tri(torch.tensor(mesh_sk_H.p).T, torch.tensor(mesh_sk_H.t).T),
    Element_Tri(P_order=k_int, int_order=q),
)

V_h = Basis(
    Mesh_Tri(torch.tensor(mesh_sk_h.p).T, torch.tensor(mesh_sk_h.t).T),
    Element_Tri(P_order=k_test, int_order=q),
)
...
for epoch in range(epochs):
    ...
    residual_value_int = V_h.integrate_lineal_form(residual, I_H_NN_grad)[
        V_h.inner_dofs
    ]
