import torch 
from fracture_fem import Fractures, Element_Fracture, Fracture_Basis, Fracture_Element_Line, Interior_Facet_Fracture_Basis
...
b = V.reduce(V.integrate_lineal_form(l, f))
...
fractures_data = torch.tensor([[[0., 0., 0.], 
                                [1., 0., 0.], 
                                [0., 1., 0.], 
                                [1., 1., 0.]]])

mesh_f = Fractures(triangulations = fractures_triangulation,
                 fractures_3D_data = fractures_data)

elements_f = Element_Fracture(P_order = 1, 
                            int_order = 2)

V_f = Fracture_Basis(mesh_f, elements_f)

A_f = V_f.reduce(V_f.integrate_bilineal_form(a))
b_f = V_f.reduce(V_f.integrate_lineal_form(l, f_3D))

u_f = torch.zeros(V_f.basis_parameters["linear_form_shape"])
u_f[V_f.basis_parameters["inner_dofs"]] = torch.linalg.solve(A_f, b_f)

V_f_inner_edges = Interior_Facet_Fracture_Basis(mesh_f, Fracture_Element_Line(P_order = 1, int_order = 2))
...
I_u_values, I_u_count = torch.unique(torch.round(I_u.reshape(-1), decimals = 12), return_counts=True)
I_u_f_values, I_u_f_count = torch.unique(torch.round(I_u_f.reshape(-1), decimals = 12), return_counts=True)

I_u_grad_values, I_u_grad_count = torch.unique(torch.round(I_u_grad.reshape(-1), decimals = 12), return_counts=True)
I_u_f_grad_values, I_u_f_grad_count = torch.unique(torch.round(I_u_f_grad[...,:2].reshape(-1), decimals = 12), return_counts=True)
...
print("Values of I_u_f_grad:", I_u_f_grad_values.numpy(),"Repeat:", I_u_f_grad_count.numpy())
print("I_u error       norm:", (torch.norm(I_u - I_u_f.squeeze(0))/torch.norm(I_u)).item())
print("I_u_grad        norm:", (torch.norm(I_u_grad - I_u_f_grad[...,:2].squeeze(0))/torch.norm(I_u_grad)).item())

eta_E_f = V_inner_edges.integrate_functional(jump, h_E_f, n_E_f, I_u_f_grad)

print("eta_E           norm:", (torch.norm(eta_E - eta_E_f.squeeze(-1))/torch.norm(eta_E)).item())
