...
V_H = Basis(Mesh_Tri(triangulation = mesh_H), Element_Tri(P_order = 2, int_order = 3))
...
mesh_h = tr.triangulate(dict(vertices = V_H.coords4global_dofs, segments = mesh_H["edges"]), "Dqepca"+str(h))

V_h = Basis(Mesh_Tri(triangulation = mesh_h), Element_Tri(P_order = 1, int_order = 3))
...
meshio.Mesh(points = mesh_H["vertices"],
            cells = [("triangle", mesh_H["triangles"])]).write("mesh_H.msh")
