...
class NeuralNetwork(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        ...
        """
        stack_input = torch.cat([x, y], dim=-1)

        output = self.activation_function(self.layer_in(stack_input))
...
class NeuralNetwork3D(torch.nn.Module):
    """
    Neural Network class for constructing a deep neural network.
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, z: torch.Tensor):
        ...
        - torch.Tensor: Output tensor after passing through the network.
        """
        stack_input = torch.stack([x, y, z], dim=-1)
