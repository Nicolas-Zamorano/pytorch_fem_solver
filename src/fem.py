...
class Abstract_Mesh(ABC):
    ...
    def __init__(self, triangulation: dict):
        ...
        Args:
            triangulation (dict): A dictionary containing mesh information with at least the following keys:
...
class Mesh_Tri(Abstract_Mesh):
    ...
    @staticmethod
    def compute_mesh_parameters(
        coords4nodes: torch.Tensor, nodes4elements: torch.Tensor
    ):
...
class Abstract_Element(ABC):
    def __init__(self, P_order: int, int_order: int):
    ...
    def compute_integral_values(self, coords4elements: torch.Tensor):
        ...
        bar_coords, v, v_grad = self.compute_shape_functions(
            self.gaussian_nodes, self.inv_map_jacobian
        )

        integration_points = torch.split(
            bar_coords.mT @ coords4elements.unsqueeze(-3), 1, dim=-1
        )
    ...
    def compute_shape_functions(
        self, gaussian_nodes: torch.Tensor, inv_map_jacobian: torch.Tensor
    ):
...
class Basis(Abstract_Basis):
    ...
    def compute_dofs(
        ...
        if P_order == 2:
            ...
            new_coords4dofs = (
                coords4nodes[edges_parameters["nodes4unique_edges"]]
            ).mean(-2)
            new_nodes4dofs = (
            ...
            new_nodes4boundary_dofs = (
                edges_parameters["nodes_idx4boundary_edges"]
                + mesh_parameters["nb_nodes"]
            )
            ...
            nodes4boundary_dofs = torch.cat(
                [nodes4boundary, new_nodes4boundary_dofs], dim=-1
            )
    ...
    def interpolate(self, basis, tensor=None):
        ...
        if basis.__class__ == Interior_Facet_Basis:
            ...
            bar_coords, v, v_grad = self.elements.compute_shape_functions(
...
class Interior_Facet_Basis(Abstract_Basis):
    def __init__(self, mesh: Abstract_Mesh, elements: Element_Line):
        ...
        self.v, self.v_grad, self.integration_points, self.dx = (
            elements.compute_integral_values(coords4elements)
        )
