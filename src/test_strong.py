...
def NN_laplacian(NN, x, y):
    ...
    du_xx = torch.autograd.grad(outputs = gradients[0],
                            inputs = (x,y),
                            grad_outputs = torch.ones_like(gradients[0]),
                            retain_graph = True,
                            create_graph = True)[0]
    
    du_yy = torch.autograd.grad(outputs = gradients[1],
                            inputs = (x,y),
...
def L2_exact(elements: Elements):
    
    x, y = elements.integration_points

    return rhs(x, y)**2

def L2_norm(elements: Elements):
   
    x, y = elements.integration_points

    NN_lap = NN_laplacian(NN, x, y)
    
    return (NN_lap + rhs(x, y))**2
...
for epoch in range(epochs):
    ...
    error_norm = torch.sqrt(torch.sum(V.integrate_functional(L2_norm)))/exact_norm
