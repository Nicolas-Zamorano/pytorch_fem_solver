...
fracture_triangulation = tr.triangulate(dict(vertices = pts, segments = segments), 
                                        "pqsea"+str(h**(n))
                                        )
...
exact_dx = lambda x, y : -y * (1 - y) * (torch.sign(x) * (x**2 - 1) + 2 * x * torch.abs(x))
...
for i in range(11):
    ...
    mesh = Mesh_Tri(fracture_triangulation_torch)
