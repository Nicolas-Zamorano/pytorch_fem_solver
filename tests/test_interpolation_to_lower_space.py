...
V_H = Basis(
    MeshTri(triangulation=mesh_H), ElementTri(polynomial_order=2, integration_order=3)
)
...
u_h = V_H.solution_tensor()

u_h = V_H.solve(A, u_h, b)

mesh_h = tr.triangulate(
    dict(vertices=V_H._coords4global_dofs, segments=mesh_H["edges"]),
    "Dqepca" + str(MESH_SIZE),
)

V_h = Basis(
    MeshTri(triangulation=mesh_h), ElementTri(polynomial_order=1, integration_order=3)
)
